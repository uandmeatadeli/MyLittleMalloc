...
mymalloc.c - Contains the custom implementation of the malloc and free functions. It includes the logic for memory allocation, deallocation, aerror handling and memory optimization strategies.
...
memgrind.c - Extensive test cases, alignment, error handeling, coalesing test, Correctness Testing, Performance Testing

memtest.c - Testing mymalloc

Hardtesting.c - This code tests the custom memory allocator by allocating multiple objects, filling them with distinct patterns, verifying the integrity of these patterns, and then deallocating the memory. The key steps include

Makefile - The makefile lol

Readme- read it. Your reading it now!
...
gcc -o memtest memtest.c mymalloc.c

gcc -o hardtesting hardtesting.c mymalloc.c
...
Objective: To test the allocators to handle ix of allocations an deallocations in a random order. 
...
Objective: Allocator performace test when working a larger mmeoery, fill up to 4064 bytes of memory.
...
Allocation: Allocate a block of random size (1 to 100 bytes) if less than 100 blocks have been allocated.
...
Average time for testMallocAndFreeImmediately: 7 microseconds
Test 2 completed: Allocated and then freed 120 1-byte objects.
Average time for testMallocStoreAndFree: 180 microseconds
Test 3 completed: Randomly allocated and freed 120 1-byte objects.
testRandomAllocFree: 84 microseconds


T1, Malloc and free 120 times
Average time for test1: 10 microseconds
T2, Malloc 120 times then free all of em
Average time test2: 178 microseconds
T3, either malloc or free then, free all
The average time is: 
Time taken is: 32.54 milliseconds
T4, mem 4064 up;
The average time is: 
Time taken is: 291.60 milliseconds
T5, fill mem to 4064 bytes, malloc and then free 120 times
The average time is: 
Time taken is: 0.54 milliseconds

Test: Freeing an address not obtained from malloc...
myfree error: Pointer 0x7ffe0d4943a4 not allocated by mymalloc (memgrind.c:746)
Test: Freeing an address not at the start of a chunk...
myfree error: Pointer 0x5595f691c04c does not match the start of any allocated chunk (memgrind.c:752)
Test: Double freeing the same pointer...
myfree error: Pointer 0x5595f691c048 does not match the start of any allocated chunk (memgrind.c:760)

#1 Coalescing Test: Attempt to allocate after freeing smaller chunks
Successfully allocated a 48-byte chunk after coalescing.
Average time for testCoalescing: 32 microseconds
#2 Test Coalesing again
Allocated 99 chunks of 24 bytes.
Freed 99 chunks.
passed.
#3 Test function for alignment requirements
...
Alignment is goodAverage time for Alignment Testing: 1 microseconds
#4 Test Randomized good
Starting randomized test...
Randomized test completed.
#5 Test if theire are Overlaps in allocation 
...
Average time for testExactFitAllocation : 1 microseconds
#8 
Test: Fragmentation Handling
Success: Managed fragmentation effectively.
Average time for testFragmentationHandling : 5 microseconds
#9 
Stress Test: Random Operations
Success: Managed Stress test.
Average time for stressTestRandomOperations : 579 microseconds
...
Allocation: It dynamically allocates memory for NUM_OBJECTS objects, each of OBJECT_SIZE bytes, using the custom malloc implementation.
Pattern Filling: Each allocated block is filled with a unique character pattern ('A', 'B', 'C', ...), which serves as a distinct identifier for that memory block.
Pattern Checking: After filling, it verifies that the pattern in each block remains intact and unchanged, indicating no memory corruption or overlap.
Deallocation: Finally, it frees the allocated memory using the custom free implementation.

Output: All patterns preserved correctly.
